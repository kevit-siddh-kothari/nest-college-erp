...
describe('UserService', () => {
  ...
  describe('signUp', () => {
    ...
    it('should throw InternalServerErrorException on failure', async () => {
      ...
      jest.spyOn(userRepo, 'addUser').mockRejectedValue(new Error('Database error'));

      await expect(service.signUp(createUserDto)).rejects.toThrow(InternalServerErrorException);
    });
  });

  describe('logIn', () => {
    it('should log in user and return token', async () => {
      const updateUserDto: UpdateUserDto = { username: 'test@mail.com', password: 'password' };
      const userEntity: UserEntity = { id: '1', username: 'test@mail.com', password: 'hashed-password' } as UserEntity;
    ...
    it('should throw NotFoundException if user not found', async () => {
      const updateUserDto: UpdateUserDto = { username: 'nonexistent@mail.com', password: 'password' };
      jest.spyOn(userRepo, 'findByUsername').mockResolvedValue(null);

      await expect(service.logIn(updateUserDto)).rejects.toThrow(NotFoundException);
    });

    it('should throw NotFoundException if password is incorrect', async () => {
      const updateUserDto: UpdateUserDto = { username: 'test@mail.com', password: 'wrong-password' };
      const userEntity: UserEntity = { id: '1', username: 'test@mail.com', password: 'hashed-password' } as UserEntity;
  ...
  describe('logOut', () => {
    ...
    it('should throw NotFoundException if user or token is missing', async () => {
      ...
      await expect(service.logOut(req, res)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('logOutAll', () => {
    it('should log out user from all devices', async () => {
      ...
      const result = await service.logOutAll(req, res);
    ...
    it('should throw NotFoundException if user is missing', async () => {
      ...
      await expect(service.logOutAll(req, res)).rejects.toThrow(NotFoundException);
    });
  });
});

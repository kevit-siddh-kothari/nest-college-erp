...
describe('StudentInfRepository', () => {
  ...
  let queryBuilder: SelectQueryBuilder<StudentEntity>;
  ...
  const mockStudent: StudentEntity = {
    ...
    phno: '1234567890',
    currentSem: '5',
    department: { id: 'dept1', name: 'Computer Science', created_at: new Date(), updated_at: new Date(), students:[], batchDetails:[] },
    batch: { id: 'batch1', year: '2021', yearId:[], created_at: new Date(), updated_at: new Date(),  students:[]},
    attendance: [],
    created_at: new Date(),
    updated_at: new Date(),
  };
  ...
  describe('getStudent', () => {
    it('should return an array of students', async () => {
      const usernameDto: CreateStudentInfDto = { username: 'john_doe@mail.com' };
      ...
      expect(mockQueryBuilder.leftJoinAndSelect).toHaveBeenCalledWith('student.batch', 'batch');
      expect(mockQueryBuilder.leftJoinAndSelect).toHaveBeenCalledWith('student.department', 'department');
      expect(mockQueryBuilder.where).toHaveBeenCalledWith('student.username = :username', { username: 'john_doe@mail.com' });
